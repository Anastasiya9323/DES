...
public class Main {
    ...
    private int[] IP_revers={40,8,48,16,56,24,64,32,
    ...
    private int[] P_box={32,1,2,3,4,5,
                         4,5,6,7,8,9,
                         8,9,10,11,12,13,
                         12,13,14,15,16,17,
                         16,17,18,19,20,21,
    ...
    public Main(String key,String content){
        this.content=content;
        p_origin_length=content.getBytes().length;
        generateKeys(key);
    }
    public static void main(String[] args) throws IOException {
        File file = new File("C:/Users/Lenovo/Desktop/DES/src/file.txt");
        FileReader file1 = new FileReader("C:/Users/Lenovo/Desktop/DES/src/file.txt");
        StringBuilder sb = new StringBuilder();
        StringBuilder sbnew = new StringBuilder();
        BufferedReader reader = new BufferedReader(file1);
        String origin;
        while ((origin = reader.readLine()) != null) {
            sbnew=sb.append(origin).append(System.lineSeparator());
        }
        origin= sbnew.substring(0,sbnew.length());
        reader.close();
        System.out.println("Оригинал: \n"+origin);
        Main customDES=new Main("Шифрование",origin);
        byte[] c=customDES.deal(origin.getBytes(),1);
        System.out.println("Шифрованный текст: \n"+new String(c));
        ...
        printWriter.print(new String(c));
        printWriter.close();
    }
    ...
    public byte[] deal(byte[] p ,int flag){
        ...
        r_num = 8- (origin_length-g_num * 8); // 8 не заполняем
        byte[] p_padding;
        /** Заливка **/
        if (r_num<8){
    ...
    public byte[] descryUnit(byte[] p,int k[][],int flag){
        ...
        if  (flag == 1) {// шифрование
                for (int i = 0; i < 16; i++) {
                        L(p_IP, i, flag, k[i]);
                }
        } else if  (flag == 0) {// расшифровать
                for (int i = 15; i > -1; i--) {
                        L(p_IP, i, flag, k[i]);
                }
        }
        int[] c=new int[64];
        for(int i=0;i<IP_revers.length;i++){
                c[i]=p_IP[IP_revers[i]-1];
        }
    ...
    public int[] fFuction(int [] r_content,int [] key){
        ...
        for(int i=0;i<P_box.length;i++){
                e_k[i]=r_content[P_box[i]-1]^key[i];
        }
        /** Замена блока S: изменить с 48 бит на 32 бит, теперь разделить e_k, а затем заменить **/
        int[][] s=new int[8][6];
        int[]s_after=new int[32];
        for(int i=0;i<8;i++){
                System.arraycopy(e_k,i*6,s[i],0,6);
                int r = (s [i] [0] << 1) + s [i] [5]; // абсцисса
                int c = (s [i] [1] << 3) + (s [i] [2] << 2) + (s [i] [3] << 1) + s [i] [4]; // ось Y
    ...
    public void generateKeys(String key){
        ...
        // k_bit - это начальный 64-битный длинный ключ, следующим шагом будет его замена
        /** Сжатие ПК-1 **/
        ...
        for(int i=0;i<16;i++){
                ...
                for  (int j = 0; j <PC2.length; j ++) {// Замена сжатия PC2
